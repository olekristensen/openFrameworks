...
// Recursive draw for instanced models
...
public:
    ...
    bool loadModel(std::string modelName, bool optimize=false, bool normalize = false);
    bool loadModel(ofBuffer & buffer, bool optimize=false, bool normalize = false, const char * extension="");
    
    //of3dPrimitive loadAs3DPrimitives(std::string modelName, bool optimize=false, bool normalize=false);
    ...
    void drawRecursive(ofPolyRenderMode renderType, aiNode * node = NULL);
    
    ofxAssimp3dPrimitive * getPrimitives(aiNode * node = nullptr, ofxAssimp3dPrimitive * parentPrimitive = nullptr);
...
protected:
    ...
    // ai scene setup
    unsigned int initImportProperties(bool optimize, bool normalize = false);
    bool processScene();
